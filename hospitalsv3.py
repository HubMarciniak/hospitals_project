...
class Space():
    ...
    def heat_map(self, filename):
        ...
        cost_size = 0
        padding = 5
        matrix = np.zeros((self.height+1, self.width+1))
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                """ Heat Distribution"""
                color = ["gray", "white", "yellow", "orange", "red", "brown", "purple", "blue"]

                "7th layer"
                if (i+7, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i-7, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i, j+7) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i, j-7) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i+6, j-1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i-6, j+1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 1
                if (i+6, j+1) in self.houses:
                ...
                "6th layer"
                if (i+6, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i-6, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i, j+6) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i, j-6) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i+5, j-1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i-5, j+1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 2
                if (i+5, j+1) in self.houses:
                ...
                "5th layer"
                if (i + 5, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i - 5, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i, j + 5) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i, j - 5) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i + 4, j - 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i - 4, j + 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 3
                if (i + 4, j + 1) in self.houses:
                ...
                "4th layer"
                if (i + 4, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i - 4, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i, j + 4) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i, j - 4) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i + 3, j - 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i - 3, j + 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 4
                if (i + 3, j + 1) in self.houses:
                ...
                "3rd layer"
                if (i+3, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i-3, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i, j+3) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i, j-3) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i+2, j-1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i+2, j+1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 5
                if (i-2, j-1) in self.houses:
                ...
                "2nd layer"
                if (i+2, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i-2, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i, j+2) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i, j-2) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i+1, j-1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i-1, j+1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 6
                if (i-1, j-1) in self.houses:
                ...
                "1st layer"
                if (i, j - 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 7
                if (i-1, j) in self.houses:
                    matrix[i, j] = matrix[i, j] + 7
                if (i, j + 1) in self.houses:
                    matrix[i, j] = matrix[i, j] + 7
                if (i+1, j) in self.houses:
                ...
                # Coloring the map
                if matrix[i, j] == 1:
                    draw.rectangle(rect, fill=color[7])
                if matrix[i, j] > 1:
                    draw.rectangle(rect, fill=color[6])
                if matrix[i, j] > 4:
                    draw.rectangle(rect, fill=color[5])
                if matrix[i, j] > 6:
                    draw.rectangle(rect, fill=color[4])
                if matrix[i, j] > 8:
                    draw.rectangle(rect, fill=color[3])
                if matrix[i, j] > 10:
                    draw.rectangle(rect, fill=color[2])
                if matrix[i, j] > 12:
                    draw.rectangle(rect, fill=color[1])

                "Homes"
                if (i, j) in self.houses:
                    draw.rectangle(rect, fill=color[0])

        img.save(filename)

# Create a new space and add houses randomly
s = Space(height=20, width=20, num_hospitals=5)
for i in range(15):
